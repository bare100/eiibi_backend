...
class NewCommentReplyNotification implements FCMNotificationItem {
  send = async (postId: string, accountId: string, initiatedBy: string) => {
    try {
      ...
      const notificationContent = await NotificationContent.findByPk(
        NotificationTypes.REPLY_ON_POST_COMMENT
      )
      if (!notificationContent || !notificationContent.enabled) {
        return
      }

      if (account.allowedNotifications.REPLY_ON_POST_COMMENT === false) {
        return
      }
      ...
      console.error('Could not send comment reply notification', error)
    }
  }
}
