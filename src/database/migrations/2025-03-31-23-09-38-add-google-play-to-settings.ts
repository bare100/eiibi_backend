...
export async function down({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    await queryInterface.removeColumn(DATABASE_MODELS.SETTINGS, 'googlePlayLink', { transaction })

    await queryInterface.removeColumn(DATABASE_MODELS.SETTINGS, 'appStoreLink', { transaction })
